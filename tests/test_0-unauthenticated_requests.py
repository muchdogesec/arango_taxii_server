...
from tests.base_test import BaseTest, BASE_URL, API_ROOT, TAXII_ENDPOINTS, LIST_OF_COLLECTION_IDS, VALID_MITRE_ATTACK_ENTERPRISE_COLLECTION_OBJECT_IDS, VALID_MITRE_ATTACK_ENTERPRISE_COLLECTION_RELATIONSHIP_IDS, VALID_MITRE_ATTACK_ENTERPRISE_COLLECTION_EMBEDDED_RELATIONSHIP_IDS, VALID_STIX_OBJECTS_FOR_POST, EXPECTED_RESPONSES, USERS
from requests.auth import HTTPBasicAuth

def create_test_method(endpoint, method, url, expected_response, json=None, auth=None, test_number=None):
    def test_method(self):
        print(f"Hitting endpoint: {url} with method: {method}")
        self.check_response(method, url, expected_response, json=json, auth=auth, test_number=test_number)
    return test_method

def create_get_test_method(endpoint, method, url, expected_response, expected_body, auth=None, test_number=None):
    def test_method(self):
        print(f"Hitting endpoint: {url} with method: {method}")
        self.check_get_response(method, url, expected_response, expected_body, auth=auth, test_number=test_number)
    return test_method
...
for endpoint_data in endpoints_to_test:
    ...
    for collection_id in LIST_OF_COLLECTION_IDS:
        ...
        if method == "POST" and collection_id == "mitre_attack_enterprise":
            ...
            test_method = create_test_method(endpoint, method, url, expected_response, json=VALID_STIX_OBJECTS_FOR_POST, test_number=test_index)
            ...
            for obj in VALID_STIX_OBJECTS_FOR_POST[0]["objects"]:
                ...
                get_test_method = create_get_test_method(endpoint, "GET", get_url, EXPECTED_RESPONSES["200"], expected_body, auth=get_auth, test_number=test_index)
            ...
            for obj in VALID_MITRE_ATTACK_ENTERPRISE_COLLECTION_OBJECT_IDS:
                final_endpoint = formatted_endpoint.replace("{OBJECT_ID}", obj["id"])
                url = f"{BASE_URL}{final_endpoint}"

                delete_test_method = create_test_method(endpoint, method, url, expected_response, test_number=test_index)
                ...
                get_auth = HTTPBasicAuth('root', USERS['root'])
                get_test_method = create_get_test_method(endpoint, "GET", get_url, EXPECTED_RESPONSES["200"], {"more": False, "next": None, "objects": [{"id": obj["id"]}]}, auth=get_auth, test_number=test_index)
            ...
            if collection_id == "mitre_attack_enterprise":
                for obj in object_ids:
                    ...
                    test_method = create_test_method(endpoint, method, url, expected_response, test_number=test_index)
...
test_method = create_test_method(random_endpoint['endpoint'], random_endpoint['method'], url, random_expected_response, test_number=test_index)
