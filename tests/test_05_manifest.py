...
@pytest.mark.parametrize(
    ...
    [
        pytest.param({'Accept': "xml"}, 'user_rw', 'test1_database', COLL_1_2, {}, 406, None, id="bad accept header"),
        pytest.param({'Accept': "application/json"}, 'user_rw', 'test1_database', COLL_1_2, {}, 406, None, id="bad accept header"),
        ...
        pytest.param(None, 'user_rw', 'test1_database', COLL_1_2, {}, 200, last_versions, id="no filter"),
...
def test_manifest(headers, user, api_root, collection_id, filter_kwargs, expected_status, expected_versions):
    s = get_session()
    s.auth = (user, user)
    resp = s.get(
        urljoin(base_url, f"api/taxii2/{api_root}/collections/{collection_id}/manifest/"),
        headers=headers,
    ...
    versions = {(x['id'], x['version']) for x in data['objects'] if not (x['id'].startswith('marking-definition') or x['id'].startswith('identity'))}
