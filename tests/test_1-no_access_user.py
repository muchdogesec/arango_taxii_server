...
class NoAccessUserRequestsTest(BaseTest):
    pass
...
auth = HTTPBasicAuth('no_access_user', USERS['no_access_user'])
...
for endpoint_data in endpoints_to_test:
    ...
    # Skip the status endpoint for no access user tests
    ...
    if endpoint == "/api/taxii2/" and method == "GET":
        ...
        setattr(NoAccessUserRequestsTest, test_method.__name__, test_method)
        test_index += 1
        continue
    ...
    for collection_id in LIST_OF_COLLECTION_IDS:
        ...
        if method == "POST" and collection_id == "mitre_attack_enterprise":
            ...
            test_method = create_test_method(endpoint, method, url, expected_response_404, json=VALID_STIX_OBJECTS_FOR_POST, auth=auth, test_number=test_index, test_label="POST objects")
            ...
            for obj in VALID_STIX_OBJECTS_FOR_POST[0]["objects"]:
                ...
                setattr(NoAccessUserRequestsTest, get_test_method.__name__, get_test_method)
                test_index += 1
            ...
            for obj in VALID_STIX_OBJECTS_FOR_POST[0]["objects"]:
                ...
                delete_test_method = create_test_method(endpoint, method, url, expected_response_404, auth=auth, test_number=test_index, test_label="DELETE objects")
                delete_test_method.__name__ = f"test_{test_index}_{method}_{final_endpoint.replace('/', '_')}"
                setattr(NoAccessUserRequestsTest, delete_test_method.__name__, delete_test_method)
                test_index += 1
            ...
            if collection_id == "mitre_attack_enterprise":
                for obj in object_ids:
                    ...
                    test_method = create_test_method(endpoint, method, url, expected_response_404, auth=auth, test_number=test_index, test_label="Generic GET/POST")
